...
class DenunciaModel extends Model
{
    ...
    {
        $builder = $this->db->table('denuncias d');
        $builder->select('cd.id, cd.nombre AS categoria, COUNT(DISTINCT sd.id) AS total_subcategorias');
        ...
        if ($filters['cliente'] !== null && $filters['cliente'] !== '') {
            $builder->where('d.id_cliente', $filters['cliente']);
        }

        if ($filters['sucursal'] !== null && $filters['sucursal'] !== '') {
            $builder->where('d.id_sucursal', $filters['sucursal']);
        }

        if ($filters['departamento'] !== null && $filters['departamento'] !== '') {
            $builder->where('d.id_departamento', $filters['departamento']);
        }
